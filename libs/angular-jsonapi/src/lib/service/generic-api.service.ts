...
export class GenericApiService {
  ...
  postFormData<R extends IResource>(
    ...
    return this.requestWithFormDataBody(
      HttpMethod.Post,
      resource,
      formData,
      ...opts,
  ...
  deleteBatch<R extends IResource>(
    resources: R[],
    ...opts: GenericApiSvcOption[]
  ): Observable<R> {
    return this.requestBatchWithBody(
      ...
      this.config.httpHeaders.keys().forEach((key) => {
        if (this.config.httpHeaders.has(key)) {
          requestHeaders = requestHeaders.set(
            key,
            this.config.httpHeaders.get(key),
          );
        }
      });
    }
  ...
  private request<T, R extends IResource>(
    ...
    const params =
      method === 'GET'
        ? new Search(...config.searchOptions).getHttpParams()
        : null;
  ...
  private requestWithBody<R extends IResource>(
    ...
    const url = this.buildUrl(
      modelType,
      this.getResourceID(method, resource),
      config.url,
    );
    const httpHeaders = this.buildHttpHeaders(config.httpHeaders);
