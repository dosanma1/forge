...
export class NodeConfigPanelComponent {
  ...
  /** Node width - all architecture nodes are 280px wide */
  private static readonly NODE_WIDTH = 280;

  /** Panel position (right edge of node) for originX: 'end' positioning */
  protected readonly panelPosition = computed(() => {
    const d = this.data();
    if (!d) return { x: 0, y: 0 };
    // Return the right edge position (node.x + width)
    return {
      x: d.position.x + NodeConfigPanelComponent.NODE_WIDTH,
      y: d.position.y,
    };
  });
  ...
  protected readonly headerBgClass = computed(() => {
    ...
    const colorScheme = this.metadataService.getColorScheme(type);
    return this.styleService.getHeaderBgClass(colorScheme);
  });
  ...
  protected readonly previewRootPath = computed(() => {
    ...
    return type
      ? this.metadataService.generateRootPath(type, kebabName)
      : kebabName;
  });
