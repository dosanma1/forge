...
import { NODE_TYPE_COMPONENTS } from './node-type-registry';
import {
  IPosition,
  GraphNode,
  GraphEdge,
...
// Re-export types from shared models for backward compatibility
export type { IPosition, GraphNode, GraphEdge, GraphNodeType };
...
export class GraphEditorComponent {
  ...
  /** Expose node positions as a reactive map (updates when nodes are dragged) */
  readonly nodePositions = computed(() => {
    const nodes = this.vflowNodes();
   ...
   * Provided via DI from the feature that uses this graph editor.
   */
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  private readonly nodeTypeToComponent: Record<string, any> =
    inject(NODE_TYPE_COMPONENTS, { optional: true }) ?? {};
