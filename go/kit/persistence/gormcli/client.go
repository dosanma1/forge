...
// Option defines the contract for options applied to a gormcli.DBClient.
type Option func(*config) error

// WithDefaultTransaction makes the gormcli.DBClient use
...
// withNowFunc makes the gormcli.DBClient use
...
// WithSingularTable makes the gormcli.DBClient to create tables
...
//
// The list of default applied options applied by gormcli.New
// (but can be overiden ):
//
...
func New(dialector gorm.Dialector, m monitoring.Monitor, traceConfig persistence.TracingConfig, options ...Option) (*DBClient, error) {
	...
	tracer := persistence.NewTracer(m.Tracer(), traceConfig)
	config.gConfig.Logger = newMonitor(m,
		append(config.monitorOpts, withLogErrSkippers(traceConfig.SpanErrSkipper()))...,
	)
	db, err := gorm.Open(
		dialector,
		config.gConfig,
	...
	err = db.Use(
		newTracingPlugin(
			traceConfig.DBName(), getDBSchema(db), tracer,
			newTracingConfig(
				traceConfig.System(), traceConfig.ExcludeQueryVars(),
				map[callbackOp]persistence.DBOp{
					callbackOpCreate: sqldb.DBOpInsert,
					callbackOpQuery:  sqldb.DBOpSelect,
					callbackOpDelete: sqldb.DBOpDelete,
					callbackOpUpdate: sqldb.DBOpUpdate,
					callbackOpRaw:    sqldb.DBOpRawSQL,
				},
			),
		),
	)
