...
// given by the sql.DB conn. Optionally, a set of gormcli.Option can be provided
...
func NewClient(connURL *url.URL, m monitoring.Monitor, options ...gormcli.Option) (*gormcli.DBClient, error) {
	if connURL == nil {
	...
	return gormcli.New(
		...
		sqldb.NewTracingConfig(
			pg.System, config.Database, connURL,
			errors.SkipErrIfOneOf(gorm.ErrRecordNotFound),
			pg.SpanErrSkipper(),
		),
		options...,
	)
