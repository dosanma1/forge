...
type wrappedMonitor struct {
	...
	logErrSkipper  errors.ErrSkipper
...
func (wm *wrappedMonitor) mustLogErr(err error) bool {
	return !wm.logErrSkipper.SkipErr(err)
}
...
func withLogErrSkippers(skippers ...errors.ErrSkipper) MonitorOption {
	return func(c *monitorConfig) {
		allSkippers := errors.SkipErrIfOneOf().
			Merge(append(skippers, c.logErrSkipper)...)
		c.logErrSkipper = allSkippers
	}
}
...
func withLogLevelFromEnv() MonitorOption {
	return func(c *monitorConfig) {
		...
		if len(dbLogLevel) < 1 {
			panic(fields.NewErrInvalidEmptyString(fields.NameConfig.Merge(fields.Name("DB_LOG_LEVEL"))))
		}
...
func newMonitor(m monitoring.Monitor, options ...MonitorOption) gormlogger.Interface {
	...
	c := &monitorConfig{logErrSkipper: errors.ErrSkipFunc(func(error) bool { return false })}
