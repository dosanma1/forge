...
func NewConsumer[T any](
	...
	if handler == nil {
		panic(fields.NewErrInvalidNil(fields.NameHandler))
	}
...
func (c *consumer[T]) Subscribe(ctx context.Context, onError func(context.Context, error)) (err error) {
	if onError == nil {
		panic(fields.NewErrInvalidNil(fields.NameCallback))
	}
...
func (c *consumer[T]) consume(
	...
	var event T
	receiveCtx, span := c.startTrace(ctx, d, "receive")

	var err error
	defer func() {
		if r := recover(); r != nil {
			c.log.ErrorContext(ctx, "amqp:consumer -> recovered from panic: %v", r)
			err = fields.NewWrappedErr("panic: %v", r)
		}
	...
	processCtx, span := c.startTrace(receiveCtx, d, "process")
...
func (c *consumer[T]) startTrace(ctx context.Context, d amqp091.Delivery, operation string) (context.Context, tracer.Span) {
	newCtx := c.tracer.Propagator().Extract(ctx, carrier.NewMapStringAnyCarrier(d.Headers))
	newCtx, span := c.tracer.Start(
		newCtx,
		tracer.WithName(fmt.Sprintf("%s %s", c.exchange.name, operation)),
		tracer.WithSpanKind(tracer.SpanKindConsumer),
	)

	span.SetAttributes(
		tracer.NewKeyValue("messaging.system", "rabbitmq"),
		tracer.NewKeyValue("messaging.operation", operation),
		tracer.NewKeyValue("messaging.consumer.id", d.ConsumerTag),
		tracer.NewKeyValue("messaging.source.name", c.exchange.name),
		tracer.NewKeyValue("messaging.source.kind", c.exchange.kind.String()),
