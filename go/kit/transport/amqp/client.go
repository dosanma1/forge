...
const (
	...
	envSvcName                 = "SVC_NAME"
	FieldNameQueue fields.Name = "queue"
)
...
func NewQueue(consumerName string, opts ...queueOption) *Queue {
	...
	if len(consumerName) < 1 {
		panic(fields.NewErrInvalidEmptyString(fields.NameName))
	}
	for _, opt := range append(defaultQueueOpts(consumerName), opts...) {
		opt(q)
	}
	if len(q.name) < 1 {
		panic(fields.NewErrInvalidEmptyString(FieldNameQueue.Merge(fields.NameName)))
	}
...
func (rk routingKey) String() string {
	...
	return strings.Join(
		kslices.Map(rk, func(rkp RoutingKeyPart) string { return rkp.String() }), ".",
	)
}
...
func (c *client) reconnect() error {
	cli, err := newClient(c.conn, c.tracer, c.log, c.exchange)
	if err == nil {
		*c = *cli
	}
	return err
