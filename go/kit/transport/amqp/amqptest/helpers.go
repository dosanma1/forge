...
type producerConstructor[P any] func(amqp.Connection, tracer.Tracer, logger.Logger) (amqp.Producer[P], error)
...
func TestEncoderAndDecoder[P, C, T any](
	...
	consumerCons func(amqp.Connection, tracer.Tracer, logger.Logger, amqp.Handler[C]) (amqp.Consumer, error),
	obj P,
	...
	prod, err := producerCons(conn, tr, log)
	...
	cons, err := consumerCons(conn, tr, log, handler)
	assert.NoError(t, err)
	wg.Add(1)
