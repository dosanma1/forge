...
export class MmcConfirmationDialogService {
	...
	open<T>(
		...
		const config: DialogConfig = {
			...
			providers: [
				{ provide: CONFIRMATION_DIALOG_DATA, useValue: options?.data },
				{
					...
					useFactory: () => new MmcConfirmationDialogRef(dialogRef),
				},
			],
		...
		const dialogRef = this.dialog.open(MmcConfirmationDialog, config);
		const mmcConfirmationDialogRef = new MmcConfirmationDialogRef(
			dialogRef,
		);
